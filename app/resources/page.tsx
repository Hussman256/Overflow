...
const resources = [
  { category: 'Books', icon: '📚', items: [
    { title: 'Inventing Bitcoin',      author: 'Yan Pritzker',    desc: 'How Bitcoin works — no coding needed. The best technical introduction for non-developers.',                    url: 'https://www.inventingbitcoin.com',              free: true  },
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg)' }}>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: 'clamp(80px,10vw,96px) 24px 80px' }}>
        ...
        <div style={{ marginBottom: 56 }}>
          <p style={{ fontSize: 12, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--text-3)', marginBottom: 12 }}>Curated</p>
          <h1 style={{ fontSize: 'clamp(40px,7vw,64px)', fontWeight: 900, letterSpacing: '-0.04em', color: 'var(--text)', lineHeight: 1.0, marginBottom: 12 }}>
            Resources
          </h1>
          ...
          {resources.map((section) => (
            <div key={section.category}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20, paddingBottom: 16, borderBottom: '1px solid var(--border)' }}>
                ...
                <h2 style={{ fontSize: 18, fontWeight: 800, color: 'var(--text)', letterSpacing: '-0.01em' }}>{section.category}</h2>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(100%, 280px), 1fr))', gap: 10 }}>
                {section.items.map((item) => (
                  ...
                  >
                    <div className="card" style={{ padding: '18px 20px', height: '100%', display: 'flex', flexDirection: 'column', gap: 10 }}>
                      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8 }}>
                        <div>
                          <h3 style={{ fontSize: 14, fontWeight: 700, color: 'var(--text)', letterSpacing: '-0.01em', marginBottom: 3 }}>{item.title}</h3>
                          <p style={{ fontSize: 12, color: 'var(--text-3)' }}>{item.author}</p>
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
                          {item.free && (
                            <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 7px', borderRadius: 4, background: 'var(--green-bg)', color: 'var(--green)', border: '1px solid rgba(34,197,94,0.2)' }}>
                              FREE
                            </span>
                          )}
                      ...
                      <p style={{ fontSize: 13, color: 'var(--text-2)', lineHeight: 1.55, flex: 1 }}>{item.desc}</p>
                    </div>
                  </a>
                ))}
              </div>
        ...
        {/* Social */}
        <div style={{ marginTop: 56, padding: '24px 28px', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 12 }}>
          <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--text-3)', marginBottom: 8 }}>Twitter / X & Nostr</p>
          ...
          <p style={{ fontSize: 12, color: 'var(--text-3)' }}>
            #Bitcoin #LightningNetwork #BitcoinAfrica #BitcoinResearch
          </p>
        </div>
      </div>
    </div>
